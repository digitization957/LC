...
(function () {
  ...
  function complianceRowHtml(c) {
    ...
    return '<div class="compliance-row' + (locked ? " locked" : " status-" + c.status) + '" ' +
      ...
      '<div class="due-block"><div class="due-label">Next due</div><div class="due-date">' + esc(fmtDate(c.nextDueDate)) + "</div></div>" + right + "</div>";
  }
  ...
  function complianceModalHtml() {
    ...
    return '<div class="overlay"><div class="modal">' +
      ...
      '<div class="field-row"><div><label>Start date</label><input type="date" id="cStart" value="' + esc(d.startDate || isoDate(new Date())) + '" /></div>' +
      '<div id="cFreqField"><label>Frequency</label><div style="display:flex;gap:8px"><input type="number" min="1" id="cFreqNum" value="' + (d.frequencyNumber || 1) + '" style="width:70px" />' +
      ssHtml("cFreqUnit", freqUnitOptionsHtml(d.category || "", d.frequencyUnit || "month"), "Search frequency...") + "</div></div></div>" +
