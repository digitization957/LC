...
$(function () {
  $(document).on("click", ".files-chip-btn", function (e) {
    ...
    var $menu = $("#filesMenu" + logId);
    var wasHidden = $menu.is("[hidden]");
    $(".files-menu").attr("hidden", true);
    if (wasHidden) $menu.removeAttr("hidden");
  });
  $(document).on("click", function (e) {
    if (!$(e.target).closest(".files-wrap").length) $(".files-menu").attr("hidden", true);
  });
});
