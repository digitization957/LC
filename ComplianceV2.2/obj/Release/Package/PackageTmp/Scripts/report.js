...
(function () {
  ...
  function complianceDetailTableHtml(rows) {
    var body = rows.map(function (row) {
      var gap = row.gapDays == null ? "—" : '<span class="gap-overdue">' + row.gapDays + "d</span>";
      return "<tr><td>" + esc(row.complianceName) + "</td><td>" + esc(row.agencyName) + "</td><td>" + esc(row.ownerName) + "</td><td>" + esc(row.deptName) +
        ...
        "<td>" + fmtDate(row.dueDate) + "</td><td>" + (row.doneDate ? fmtDate(row.doneDate) : "—") + '</td><td class="num">' + gap + "</td></tr>";
    }).join("");
    return '<div class="table-wrap"><table class="owner-summary-table">' +
      "<thead><tr><th>Compliance</th><th>Agency</th><th>Owner</th><th>Dept</th><th>Status</th><th>Due Date</th><th>Done Date</th><th class=\"num\">Gap</th></tr></thead>" +
